    ...
    <div class="card border border-base-300 bg-base-200 mb-6">
        <div class="card-body p-5 gap-2">
            <div class="flex flex-wrap items-center gap-3">
                ...
                <span class="text-sm text-base-content/60">{{ $berita->created_at->locale('id')->isoFormat('D MMMM YYYY HH:mm') }}</span>
            </div>
            @if($berita->author)
    ...
    @if($berita->tags->isNotEmpty())
        <div class="flex flex-wrap gap-2 px-2 mb-6">
            @foreach($berita->tags as $tag)
                <span class="badge badge-outline font-medium">{{ $tag->tagline }}</span>
            @endforeach
        </div>
    @endif
