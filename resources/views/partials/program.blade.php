            ...
            <div id="program-carousel"
                ...
                @foreach($programs as $index => $program)
                    <div id="program-slide-{{ $index }}"
                         class="carousel-item w-full max-w-xs md:basis-1/3 xl:basis-1/4 shrink-0 snap-start">
                        <a href="{{ $program['link'] ?? '#' }}"
                           class="rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-shadow bg-white border border-gray-100 flex flex-col w-full group">
                            @if(!empty($program['image']))
                                <img src="{{ asset($program['image']) }}"
                                     alt="{{ $program['title'] }}"
                                     class="w-44 h-44 object-cover group-hover:scale-105 transition-transform duration-300">
                            @else
