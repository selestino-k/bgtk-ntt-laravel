...
@section('title', 'Siaran Pers | BGTK Provinsi NTT')

@section('content')
@include('home.partials.header')

<div id="siaran-pers" class="mt-20 w-full px-4 md:px-10 font-montserrat">
    <main class="relative z-10 py-10 w-full">
        ...
        {{-- Search Bar --}}
        <form method="GET" action="{{ route('siaran-pers.index') }}" class="mb-8">
            <div class="join w-full max-w-lg">
                ...
                @if(!empty($search))
                    <a href="{{ route('siaran-pers.index') }}" class="btn btn-ghost join-item">Reset</a>
                @endif
            </div>
        </form>
        ...
        @if($beritas->isEmpty())
            <div class="flex flex-col items-center justify-center py-20 text-base-content/50">
                <i class="fa-solid fa-microphone text-5xl mb-4"></i>
                <p class="text-lg">Tidak ada siaran pers yang ditemukan.</p>
            </div>
        @else
